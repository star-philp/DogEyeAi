...
model_path = '/Users/rainstar/Project/Python/DogAI/models/best_model.pkl'# Check if the model file existsifnot os.path.isfile(model_path):
...
defpreprocess_image(image):
    ...
    image = image.unsqueeze(0)  # Add batch dimensionreturn image
...
defgenerate_heatmap(image):
    ...
    heatmap = (heatmap[..., :3] * 255).astype(np.uint8)  # Convert to RGB# Convert to PIL imagereturn Image.fromarray(heatmap)

defoverlay_heatmap_on_image(original_image, heatmap_image):
    """
...
sample_image_path = 'path/to/your/sample_image.jpg'# Update this path with the actual path to your sample imageif os.path.isfile(sample_image_path):
...
if uploaded_file isnotNone:
    ...
    # Analyze Imagetry:
        ...
        result_text = f"Analysis Output:\nClass: {pred_class}\nIndex: {pred_idx}\nProbabilities: {probs}"# Display results in a text box
        ...
        # Save results to DB (Placeholder)# Note: Add your database saving logic here

        st.success("Analysis complete. Results have been saved.")
    except Exception as e:
        st.error(f"An error occurred during analysis: {e}")

    # User Profile Page# Note: Add your logic for displaying stored results here